...
class Baseliner:
    ...
    def __init__(self, ax, spec):
        ...
        self.__specplot, = self.__ax.plot(
            self.__x,
            self.__y,
            'k-', drawstyle='steps-mid')
        self.__buttonListener = self.__ax.figure.canvas.mpl_connect(
            'button_press_event',
            self.__mouse_press)
        self.__keyListener = self.__ax.figure.canvas.mpl_connect(
            'key_press_event',
            self.__key_press)
        self.windows = []
    ...
    def __key_press(self, event):
        if event.key == 'q':
            self.__skip()
        if event.key == 'a' and \
                self.__limlo is not None and \
                self.__limhi is not None:
            self.__addwindow(self.__limlo, self.__limhi)
            self.__ax.plot(
                [self.__limlo, self.__limlo],
                [self.__miny, self.__maxy], 'g-')
            self.__ax.plot(
                [self.__limhi, self.__limhi],
                [self.__miny, self.__maxy], 'g-')
            self.__remlim()
            self.__remlim()
    ...
    def __setlim(self, i_x):
        if self.__limlo is None:
            self.__limlo = i_x
            self.__limloplot, = self.__ax.plot(
                [i_x, i_x],
                [self.__miny, self.__maxy], 'b-')
            self.__ax.figure.canvas.draw()
        elif self.__limhi is None:
            self.__limhi = i_x
            self.__limhiplot, = self.__ax.plot(
                [i_x, i_x],
                [self.__miny, self.__maxy], 'b-')
            self.__ax.figure.canvas.draw()
            print('Ready for finalising. \
                Press once more to do so, \
                or press a to add another window.')
        else:
            self.__finalise()
...
equivalencies_absorption = [
        ...
        (
            unit_od,
            unit_abs,
            lambda x:x/np.log(10),
            lambda x:x*np.log(10)),
        (
            unit_od,
            unit_t,
            lambda x:10**(-x/np.log(10)),
            lambda x:-np.log10(x)*np.log(10))
        ]
...
def kramers_kronig(
    ...
    if type(freq) != u.quantity.Quantity:
        warnings.warn(
            'No units detected in input freq. Assuming kayser.',
            RuntimeWarning)
        freq *= u.kayser
    else:
    ...
    if type(transmittance) != u.quantity.Quantity:
        warnings.warn(
            'No units detected in input transmittance. ' +
            'Assuming transmittance units.',
            RuntimeWarning)
    ...
    if type(d_ice) != u.quantity.Quantity:
        warnings.warn(
            'No units detected in input d_ice. Assuming centimeters.',
            RuntimeWarning)
        d_ice *= u.cm
    else:
        d_ice = d_ice.to(u.cm)
    ...
    if type(m_guess) == complex:
        ...
        m_ice = m_guess
    # find top and bottom fraction indices.
    # These will be replaced with dummy values after each integration
    ...
    # pre-calculate the large denominator component of
    # the KK integration, if desired
    if precalc:
        try:
            ...
            precalc = True
        # or at least try to do so; if run out of memory,
        # switch to the slower no-precalc mode
        except MemoryError:
            precalc = False
    ...
    while squaresum_diff > tol and niter < maxiter:
        ...
        t01, t02, t12, r01, r02, r12 = \
            complex_transmission_reflection(m_vacuum, m_ice, m_substrate)
        # the reflection component
        # reflection_component =
        #   np.abs(
        #     (t01*t12/t02)/(1.+r01*r12*np.exp(4.j*np.pi*d_ice*m_ice*freq))
        #   )**2.)
        # this is an evil equation. do NOT touch it
        # it calculates the lambert absorption coefficient using
        # the current best guess at m_ice
        alpha = (1./d_ice) * \
                (-np.log(transmittance) +
                    np.log(np.abs(
                        (t01*t12/t02) /
                        (1.+r01*r12*np.exp(4.j*np.pi*d_ice*m_ice*freq))
                    )**2.))
        if np.any(np.logical_or(
                np.isnan(alpha),
                np.logical_not(np.isfinite(alpha)))):
            raise KKError(
                'Incoming transmittance data produced impossible ' +
                'lambert absorption coefficients. ' +
                'Check your input parameters.')
        # using the new alpha, calculate a new n (and thus m) for the ice
        # this is done in a parallel for loop, to avoid killing the
        # computer when dealing with large amounts of data
        ...
        for current_col in range(numcols):
            if precalc:
                kkint[current_col] += kkint_mul * \
                    scipy.integrate.simps(
                        (alpha-alpha[current_col]) *
                        kkint_deno1[current_col, :] -
                        kkint_nomi*kkint_deno2)
            else:
                kkint_deno1 = freq[current_col]**2-freq**2
                kkint_deno1[kkint_deno1 != 0] = 1. / \
                    kkint_deno1[kkint_deno1 != 0]
                kkint[current_col] += kkint_mul * \
                    scipy.integrate.simps(
                        (alpha-alpha[current_col]) *
                        kkint_deno1-kkint_nomi/(freq**2-freq_m0**2))
        if np.any(kkint < 1):
            if np.any(kkint < 0):
                warnings.warn(
                    'KK integration is producing negative ' +
                    'refractive indices! ' +
                    'This will most likely produce nonsensical results.',
                    RuntimeWarning)
            else:
                warnings.warn(
                    'KK integration is producing refractive ' +
                    'indices below unity! This may result in ' +
        ...
        if np.any(np.isnan(m_ice.real)) or np.any(np.isnan(m_ice.imag)):
            raise KKError(
                'Produced complex refractive index contains NaNs. ' +
                'Check your input parameters.')
        # replace top and bottom fractions of m_ice with the value closest
        # to that edge. this is done to combat edge effects arising from
        # integrating over a non-infinite range
        ...
        # in these 0 means vacuum, 1 means ice, 2 means substrate
        t01, t02, t12, r01, r02, r12 = complex_transmission_reflection(
            m_vacuum, m_ice, m_substrate)
        # model a transmittance using given m_ice and alpha
        # yes, this is another evil equation
        transmittance_model = \
            np.exp(-alpha*d_ice) * \
            np.abs((t01*t12/t02) /
                   (1.+r01*r12*np.exp(4.j*np.pi*d_ice*m_ice*freq)))**2.
