...
class Fitter():
    ...
    def add_empirical(self, spectrum, params, funcname=None, color='red'):
        ...
        if not np.all(spectrum.x.value == self.target_x):
            raise RuntimeError(
                'Input spectrum x axis does not ' +
                'match the target spectrum x axis.')
        self.funclist.append({
            'type': 'empirical',
            'shape': spectrum.y.value,
            'params': params,
            'name': funcname,
            'color': color})
    ...
    def fitresults_tofile(self, filename, detection_threshold=5.0):
        ...
        np.savetxt(
            filename_csv,
            outdata_csv.transpose(),
            delimiter=',', header='For info, see '+filename_xml)
    ...
    def fit_results(self):
        ...
        for indFunc, cFunc in enumerate(self.funclist):
            ...
            while oResults.__contains__(oKeyname):  # In case of duplicates
                oKeyind += 1
